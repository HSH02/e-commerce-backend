...
public class AdminController  implements AdminAPiSpecification{
    ...
    public RsData<Product> addProduct(AddProductRequest request) {
        Product product = productService.addProduct(request);
        return RsData.success(HttpStatus.CREATED, product, "상품 등록이 완료되었습니다");
    }

    @PutMapping("/products/{productId}")
    public RsData<Product> updateProduct(@PathVariable Long productId, UpdateProductRequest request) {
        Product product = productService.updateProduct(productId, request);
        return RsData.success(HttpStatus.OK, product, "상품 수정이 완료되었습니다");
    }

    @PutMapping("/products/{productId}/stock")
    public RsData<Product> manageProductStock(@PathVariable Long productId, UpdateStockRequest request) {
